...
@Script.Manifest(name="BF0Buy", description = "Money making",properties = "autor: Pos yo!")
public class Buy extends PollingScript<ClientContext> implements PaintListener {
    int widget=300,component=16, item=886, npc=1309, cantidad=0,cant=1460,widgetb=0,componentb=1,count=0,count1=0;    //npc=536
    String est="";
    int[] worlds={301,308,316,326,335,371,82,83,84,85,93,94}, ex={81,85},
    ...
    public void hop(){
        ...
        while(ctx.worlds.open()){
            ...
            for(int i=0;i<worlds.length;i++){
                ...
                if(world==94){
                    ctx.widgets.widget(69).component(8).component(c[0]).click();
                    world=1;
                    count++;
                    Condition.wait(new Callable<Boolean>() {
                        @Override
    ...
    public void stay(){
        ctx.worlds.select().types(World.Type.FREE).joinable().poll().hop();
        detener();
        ...
        for(int i=0;i<ctx.widgets.widget(69).component(8).componentCount();i=i+6){
            if((ctx.widgets.widget(69).component(8).component(i).textColor()==901389)) {
                WolrdStay = Integer.parseInt(ctx.widgets.widget(69).component(7).component(i + 2).text());
                world=i;
            }
