 ...
 */
@Script.Manifest(name="000MineMotherRenove", description = "Money making",properties = "autor: Pos yo!")
public class MotherMineRenove extends PollingScript<ClientContext> implements PaintListener {
    ...
    public void start() {
        ...
        Condition.wait(new Callable<Boolean>() {
            ...
            public Boolean call() throws Exception {
                return ctx.camera.pitch() > 94 && ctx.camera.pitch() < 100;
            }
        }, 200, 20);
    }
    public void close(){
    ...
    public void hop(){
        ...
        Condition.wait(new Callable<Boolean>() {
            ...
            public Boolean call() throws Exception {
                ...
                return ctx.widgets.widget(69).component(10).component(0).visible();
            }
        }, segundos(), 25);
    ...
    public void minar(){
        System.out.println("Empezare a minar como perra");
        Iterator<GameObject> Minas=ctx.objects.select(10).name("Ore vein").nearest().iterator();
        while(Minas.hasNext()){
            mina= Minas.next();
            ...
            if(Area_Mine.contains(mina.tile())){
                if(mina.interact("Mine")){
                    ...
                    final int random= org.powerbot.script.Random.nextInt(3,6);
    ...
    public void Caminar_Mina(){                     // requiere mejorar
        cuadro=areas.area2(Bank);
        if(cuadro.contains(ctx.players.local())) {
            ctx.camera.angle(org.powerbot.script.Random.nextInt(92, 99));
            Condition.wait(new Callable<Boolean>() {
                @Override
                public Boolean call() throws Exception {
                    System.out.println("Esperando ir al centro");
                    if (detener()) return true;
                    return ctx.camera.yaw() > 90 && ctx.camera.yaw() < 100;   //esperare a que llegue a la piedra
                }
            }, time, 10);
        }
        ...
        if(org.powerbot.script.Random.nextBoolean()) {
            ...
            if(org.powerbot.script.Random.nextBoolean()) {
                cuadro = areas.area2(new Tile(3714, 5640, 0));
                ctx.movement.step(new Tile(3714, 5640, 0));
                time = org.powerbot.script.Random.nextInt(800, 1200);
                Condition.wait(new Callable<Boolean>() {
                    @Override
                    public Boolean call() throws Exception {
                ...
                cuadro = areas.area2(new Tile(3722, 5635, 0));
                ctx.movement.step(new Tile(3722, 5635, 0));
                time = org.powerbot.script.Random.nextInt(800, 1200);
                Condition.wait(new Callable<Boolean>() {
                    @Override
                    public Boolean call() throws Exception {
    ...
    public void Bank(){
        ...
        if(ctx.bank.opened()){      //Si el bank esta abierto hare lo que esta debajo
            if(ctx.inventory.select().count()>20) {    // si inventario == 28
                nug=ctx.bank.select().id(nugID).count(true);
