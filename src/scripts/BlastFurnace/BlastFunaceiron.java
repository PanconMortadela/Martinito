...
//@Script.Manifest(name="0BlastFurnaceIron", description = "Money making",properties = "autor: Pos yo!")
public class BlastFunaceiron extends PollingScript<ClientContext> implements PaintListener{
    ...
    public void estado(){
        ...
        if(est==0&& (ctx.inventory.select().count()<28||ctx.inventory.select().id(2351).count()>1)){
            bank(); est++;}
    ...
    int time=Random.nextInt(100,300);
    ...
    public void bank(){
        ...
        if(ctx.inventory.select().id(2351).count()>=1)
            count=count+ctx.inventory.select().id(2351).count();// aumenta barras
        while(ctx.inventory.select().id(2351).count()>=1){ //mientras tenga barras de steel
            if(detener())break;;
            ctx.bank.deposit(2351,200000);
            time=Random.nextInt(100,300);
            Condition.wait(new Callable<Boolean>() {
                @Override
                public Boolean call() throws Exception {
    ...
    public void recoger(){
        ...
        while((ctx.inventory.select().count()==0&&t==false)||(ctx.inventory.select().count()<=2&&t==true)) {
            if(detener())break;
