...
public class pmcanifi extends PollingScript<ClientContext> implements PaintListener{
    ...
    boolean fail,condicion=false;
    @Override
    public void poll() {
        if(m==29||m==59){
            condicion=true;
        }
        if((m==30||m==00)&&condicion==true){
            hop();
            condicion=false;
        }
        count=estoy();
        ...
        int time=Random.nextInt(500,1200);
