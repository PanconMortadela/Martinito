...
public class Canifi2 extends PollingScript<ClientContext> implements PaintListener {
    ...
    public void estados(){
        ...
        if(count==0) {
            punto.bounds(148, 216, -292, -156, -32, 16);
            ...
            if(punto.interact(accion[count])){
                ...
                Condition.wait(new Callable<Boolean>() {
                    ...
                    public Boolean call() throws Exception {
                        ...
                        if(ctx.players.local().animation()!=-1){
                            if(ctx.input.move(inicio[next].matrix(ctx).centerPoint()));
                        }
                ...
                Condition.wait(new Callable<Boolean>() {
                    ...
                    public Boolean call() throws Exception {
                        ...
                        if(move==true){
                            if(ctx.input.move(inicio[count].matrix(ctx).centerPoint())){
                                ctx.objects.select().id(id[count]).poll().interact(accion[count]);
                                Condition.wait(new Callable<Boolean>() {
                                    @Override
                                    public Boolean call() throws Exception {
                                        System.out.println("Voy a esperar 2");
                                        ...
                                        return cuadro.contains(ctx.players.local().tile())||areas.area3(llegada[count]).contains(ctx.players.local().tile());
                                    }
                                }, 1000, time);
                                move=false;
        ...
        exptake=ctx.skills.experience(Constants.SKILLS_AGILITY);
