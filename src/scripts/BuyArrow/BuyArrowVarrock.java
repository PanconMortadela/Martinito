 ...
 */
@Script.Manifest(name="00BuyVarrock", description = "Money making",properties = "autor: Pos yo!")
public class BuyArrowVarrock extends PollingScript<ClientContext> implements PaintListener {
    int widget=300,component=16, item=886, npc=2883, cantidad=0,cant=920,widgetb=0,componentb=3,count=0,count1=0;    //npc=536
    String est="";
    ...
    public void compra(){
        ...
        while(ctx.widgets.widget(widget).component(component).component(componentb).itemStackSize()>cant){
            ...
            if(ctx.widgets.widget(widget).component(component).component(componentb).itemStackSize()>1495)
                ctx.widgets.widget(widget).component(component).component(componentb).interact("Buy 10"); // cambie el de 50 por 10
            else
